...
float cmin(float a, float b, float c){
    if(a < b){
        if(a < c){
            return a;
        }
...
void process (const char* file_1, const char* file_2, const char* outfile, int maxdisp, int no_interp) 
{
    ...
    float* cost = (float*)malloc(width*height*sizeof(float));
    float* track = (float*)malloc(width*height*sizeof(float));
    ...
    for(int row = 0; row < height; row++){
        for(int col = 0; col < width; col++){
            cost[width * col + 1] = col * occ_cost;
        }
        for(int col = 0; col < width; col++){
            cost[width + col] = col * occ_cost;
        }
        for(int i = 1; i < width; i++){
            for(int j = 1; j < width; j++){
                float temp = (I1->data[width * row + i] - I2->data[width * row + j]) * (I1->data[width * row + i] - I2->data[width * row + j]);
                float min1 = cost[(i - 1) * width + j - 1] + temp;
                float min2 = cost[(i - 1) * width + j] + occ_cost;
                float min3 = cost[i * width + j - 1] + occ_cost;
                float temp_min = cmin(min1,min2,min3);
                cost[i * width + j] = temp_min;
                if(temp_min == min1){
                    track[i * width + j] = 1;
                }
                else if(temp_min == min2){
                    track[i * width + j] = 2;
                }
                else{
                    track[i * width + j] = 3;
        ...
        while(i != 0 && j != 0){
            switch(track[i * width + j]){
                case 1:
                    D1_data[row * width + i] = my_abs(i - j);
                    D2_data[row * width + j] = my_abs(j - i);
                    i--;j--;
                    break;
                case 2:
                    D1_data[row * width + i] = 0;
                    i--;
                    break;
                case 3:
                    D2_data[row * width + j] = 0;
                    j--;
                    break;
                default:
                    break;
